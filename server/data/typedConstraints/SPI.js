    ...
    connect(childs) {
        ...
        for (const child of childs) {
            ...
            if (parent.arch == null || child.arch == null) {
                console.warn('Unknown SPI master-slave architecture, doing nothing', '- parent arch:', parent.arch, '- child arch:', child.arch);
            }
            else {
                if (!(parent.arch == 'master' && child.arch == 'slave')) {
                    error(`SPI parent and child architecture must be of type master-slave. Parent arch:
